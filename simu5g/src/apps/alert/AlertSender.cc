...
void AlertSender::initialize(int stage)
{
    ...
    getContainingNode(this)->subscribe("alertSendMsg", this);
    numSent = 0;
...
void AlertSender::receiveSignal(cComponent *source, simsignal_t signalID, cObject* obj, cObject *details)
{
    // #ifdef TRUST_MANAGEMENT_ENABLED
    /* Currently just sending a self message to trigger an alert message. TODO: Later can change to creating different alert messages as needed */
    numSent++;
    std::cout << simTime() << ", " << numSent << std::endl;
    // #endif
    handleMessage(selfSender_);
}
